...
export const TopTvSeries = () => {
  const {
    isLoading,
    isFetching,
    data: topTvSeriesData,
    error,
    isError,
  } = useQuery(
    'search',
    async () => {
      return topResultsResponse('anime', 'tv');
    },
    { refetchOnWindowFocus: false }
  );

  const topTvSeriesResults = _.map(topTvSeriesData, (anime) => {
    const epsCheck = () => {
  ...
  return (
    <Box>
      ...
      <UnorderedList>{topTvSeriesResults}</UnorderedList>
    </Box>
  );
};

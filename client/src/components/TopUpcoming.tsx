...
export const TopUpcoming = () => {
  const {
    isLoading,
    isFetching,
    data: topUpcomingData,
    error,
    isError,
  } = useQuery(
    'search',
    async () => {
      return topResultsResponse('anime', 'upcoming');
    },
    { refetchOnWindowFocus: false }
  );

  const topUpcomingResults = _.map(topUpcomingData, (anime) => {
    const epsCheck = () => {
  ...
  return (
    <Box>
      ...
      <UnorderedList>{topUpcomingResults}</UnorderedList>
    </Box>
  );
};
